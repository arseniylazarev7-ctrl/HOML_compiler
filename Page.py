class Page:
    ...
    def gen_class_page(self, els, vars, init, update):
        ...
        self.gen_class_pageH(self, els, vars, init, update)
        self.gen_class_pageCpp(self, els, vars, init, update)
    ...
    def gen_class_pageH(self, els, vars, init, update):
        ...
        for var in vars:
            if not var.is_out:
                ...
                self.code_classH += f"\t{var.type} {var.name} = {var.name}_value;\n"
                self.includes_items += f"\n#include \"{var.name}_value.h\"\n"
        ...
        for el in els:
            if not el.is_out:
                ...
                self.code_classH += f"\t{el.type} {el.id} = {el.id}_value;\n"
                self.includes_items += f"\n#include \"{el.id}_value.h\"\n"
