...
from text_funces import del_garbage, split_last_char, get_staples, tokenize, correct_path
...
def build_var(var, path):
    if var.is_out:
        with open(f"{path}\\{split_last_char(var.text, ["\\", "/"])[0]}\\{var.name}_value.h", "w") as f:
            if var.value != "":
                f.write(f"#pragma once\n{var.includes}\n{var.type} {var.name}_value = {var.value};\n")
            else:
                f.write(f"#pragma once\n{var.includes}\n{var.type} {var.name}_value;\n")

def build_el(el, path):
    if el.is_out:
        with open(f"{path}\\{split_last_char(el.text, ["\\", "/"])[0]}\\{el.id}_value.h", "w") as f:
            f.write(f"#pragma once\n{el.type} {el.id};\n")
...
def build_app():
    ...
    with open(App.path + f"\\dist\\{App.ID}\\ClassApp{App.ID}.h", "w") as f:
        ...
        for page in App.Pages:
            ClassAppAppH += f"\n// These includes are for page, with id \"{page.ID}\"\n"
